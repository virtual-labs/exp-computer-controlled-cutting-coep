...
WoodCuttingObj.LevelOneConfig = function(){
			...
			WC_level1 +='<div class="row">' 
				...
				+'<div class="row ">'
				        ...
				        + '<option value="600-400">'+(8 * 304.8).toFixed(0)+' * '+(4 * 304.8).toFixed(0)+' (mm)</option>'
				        + '<option value="500-500">'+(5 * 304.8).toFixed(0)+' * '+(5 * 304.8).toFixed(0)+' (mm)</option>'
				        + '<option value="800-400">'+(12 * 304.8).toFixed(0)+' * '+(4 * 304.8).toFixed(0)+' (mm)</option>'
...
//						
						...
						+'<h1>Wood Cutting</h1>'    
//						+'<img src="simulation/images/MC_BG2.png" style="position: absolute;top: 120px;" id="WCMachinImg">'
						+'<img id="WCMachinImg">'
	 	...
	 	WoodCuttingObj.LevelOneSubmit = function(){
			 ...
			 if(WoodSize != -1 && Thickness != -1 )
				...
				// WCJson.WC_Origin_Y  = 0;					 
			     ...
			     if(parseInt(WoodSize.split('-')[1]) == 500)
			    	 {
			    	 $("#WCMachinImg").attr('src', 'simulation/images/MC_BGW500.png').css({"position":"absolute","top":"97px"});
			    	 }
			     if(parseInt(WoodSize.split('-')[1]) == 400)
		    	 {
			    	 $("#WCMachinImg").attr('src', 'simulation/images/MC_BGW400.png').css({"position":"absolute","top":"83px"});
		    	 }
...
//	 		if(WCJson.WC_Redius WCJson.WC_Origin_X WCJson.WC_Origin_Y)
	 		...
	 		if (WCJson.hasOwnProperty('WC_Redius')) {
	 			...
	 			if (WCJson.hasOwnProperty('WC_PolySide')) {
	 				delete WCJson["WC_PolySide"];
	 				polySizeFlag = false ; 
	 			}
	 			if (WCJson.hasOwnProperty('WC_PolyRedius')) {
	 				delete WCJson["WC_PolyRedius"];
...
//			$("#WCLevelTwo, #WCSquareDraw, #WCEllipseDraw").prop("disabled", true);
		    ...
		    +'<p><a href="https://drive.google.com/file/d/1OZZZTyqW2aK04O6TEmdVbi0I8Fd2PESn/preview" target="_blank">Referance video of actual machine </a></p>'
		    +'<p>Cutting Depth</p>'
...
//			 +'<h1>Tool</h1>'
			+'<div class="ToolSection row">'
			 ...
			 +'</div>'
			 +'<div class="col-sm-12 col-md-12 col-xl-12">'
			 +'<center><button type="button" class="btn btn-success" data-dismiss="modal" id="WCAmimationBtn" disabled>Start Cutting</button>'
			 +'<button type="button" class="btn btn-info" id="WCReload" disabled>Reload</button></center>'
			+'</div>'
			...
			+'</div>'
					...
					for(var t = 0; t < ToolList.length; t++)
						{
						WCuttingCinfigHtm += '<option value="'+ToolList[t]+'"> '+ToolList[t]+'</option>'
						}
...
//		    			+'<div id="ToolInfo"></div>'
			...
			$('#ToolList option[value="Tapered Ball Nose 1/8*   77-102"]').attr('selected','selected');
			...
			$('input[name="CuttingDirection"]').click(function(){
				$("#DirectionError").html('');
			});
...
//						WoodCuttingObj.DisabledFields(triger);	
				...
				$("#CutDepth").prop("disabled",true);
				$('input[name="CuttingDirection"]').attr("disabled",true);
				$("#WCAmimationBtn").prop("disabled",true);
				$("#SelectToolBtn").prop("disabled",true);
				
				    console.log(WCJson);
				     $("#WCReload").prop("disabled",false);
